...
def shortest_path(source, target):
    ...
    explored_nodes = []
    frontier = QueueFrontier()  # Using a queue for BFS so first in first out
    ...
    while frontier.empty() is False:
        ...
        print("explored: ", [node.state for node in explored_nodes])

        # explore frontier and remove it
        current_node = frontier.remove()
        print("current node: ", current_node.state)
        current_person_id = current_node.state
        explored_nodes.append(current_node)

        if target == current_person_id:
            print("found target")

            # now we gotta recontruct the path with source -> target with [(current_movie_id, person_id), ...]
            path = []
            this_node = current_node
            while this_node.parent is not None:
                path.append((this_node.action, this_node.state))
